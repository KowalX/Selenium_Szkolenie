package SeleniumWorkshop.patch.org.easetech.easytest.loader;
...
public final class DataLoaderUtil {
	...
	private static String getTestDataFileExpression(String pathExpression) {
		return getTestDataFileExpression(pathExpression, System.getProperties());
	}

	public static String getTestDataFileExpression(String pathExpression, Properties properties) {
		String variable = pathExpression.substring(pathExpression.indexOf("{") + 1, pathExpression.indexOf("}"));
		String systemVariableRaw = pathExpression.substring(pathExpression.indexOf("{") - 1, pathExpression.indexOf("}") + 1);
		if (properties.get(variable) == null) {
			fail("property " + variable + " was not defined.");
		}
		String fileExpression = pathExpression.replace(systemVariableRaw, properties.get(variable).toString());
		return fileExpression == null ? variable : fileExpression;
	}
	...
	private static Loader getCustomLoaderInstance(DataLoader testData) {
		...
		if (testData.loader() == null) {
			Assert.fail("Specified the LoaderType as CUSTOM but did not specify loader" + " attribute. A loaderType of CUSTOM requires the loader "
					+ "attribute specifying " + "the Custom Loader Class which implements Loader interface.");
		} else {
			try {
				Class<? extends Loader> loaderClass = testData.loader();
				dataLoader = loaderClass.newInstance();
			} catch (Exception e) {
				throw new RuntimeException("Exception occurred while trying to instantiate a class of type :" + testData.loader(), e);
			}
		}
		return dataLoader;
	...
	/**
	 * Load the Data for the given class or method. This method will try to find {@link DataLoader} on either the class level or the method level. In case the
	 * annotation is found, this method will load the data using the specified loader class and then save it in the DataContext for further use by the system.
	 * We also create another copy of the input test data that we store in the {@link DataDrivenTestRunner#writableData} field. This is done in order to
	 ...
	 * @param currentTestClass the currently executing test class. this is used to append in front of the method name to get unique method names as there could be methods in
	 *                         different classes with the same name and thus we want to avoid conflicts.
					...
					for (String filePath : testInfo.getFilePaths()) {
						...
						try {
							if (resource.exists()) {
								...
								LOG.warn("Resource {} does not exists in the specified path. If it is a classpath resource, use 'classpath:' "
										+ "before the path name, else check the path.", resource);
							}
						} catch (Exception e) {
							LOG.error("Exception occurred while trying to load the data for resource {}", resource, e);
	...
	private static void fillWritableData(Map<String, List<Map<String, Object>>> writableData, Map<String, List<Map<String, Object>>> availableData,
										 boolean appendData) {
		if (!appendData) {
